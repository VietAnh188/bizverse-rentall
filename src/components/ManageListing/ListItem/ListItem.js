...
class ListItem extends React.Component {
  ...
  removeItem(listId) {
    return (
      <li className={s.panelBodyAlert}>
        <div className={cx(s.alertBlock, s.bizverseBox, s.bizverseBoxSecondary)}>
          <div>
            ...
            <div>
              ...
              <Button
                className={cx(s.button, s.bizverseButtonSecondary, bt.btnLarge, s.smButton, s.marginTopMb)}
  ...
  render() {
    ...
    let updatedDate = moment(data.updatedAt).format('MMM DD, YYYY');
    let listId = data.id;
    ...
    if (isRemove) {
      ...
      return (
        <li className={cx(s.panelBody, s.bizverseBorder, s.bizverseBorderTop)} >
          <Row>
            ...
            <Col xs={12} sm={12} md={7} lg={8} className={s.listDetailContent}>
              ...
              <Col xs={12} sm={12} md={10} lg={10} className={cx(s.noPadding, s.spaceTop2, s.space2)}>
                ...
                {locale === 'vi' ? (
                  <span className={s.labelText}><FormattedMessage {...messages.progressBarText1} /> <FormattedMessage {...messages.progressBarText2} /> {this.percentage(data)}%</span>
                ) : (
...

              {
                data && data.listingSteps && data.isReady && <a
                  ...
                  target="_blank"
                  className={cx('btn btn-default', s.button, bt.btnPrimaryBorder, s.spaceTop2, s.smButton, s.spaceRight2, s.bizverseLink, 'spaceRight2AR')}
                >
