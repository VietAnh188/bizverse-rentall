...
import DeleteWishListGroupForm from '../DeleteWishListGroupForm/DeleteWishListGroupForm';
...
class WishListGroupModal extends Component {
  ...
  render() {
    const { closeWishListGroupModal, actionType, initialValues } = this.props;
        ...
        <Modal show={wishListGroupModalStatus} onHide={closeWishListGroupModal} dialogClassName={cx(s.logInModalContainer, 'loginModal', 'wishListCloseBtn')} >
          ...
          <Modal.Body bsClass={s.logInModalBody}>
            ...
            <h4 className={s.titleBold}>
              {
                actionType != 'edit' && actionType !='delete' && <FormattedMessage {...messages.createWishList} />
              }
              {
                actionType == 'edit' && <FormattedMessage {...messages.editWishList} />
              }
              {
                actionType == 'delete' && <FormattedMessage {...messages.deleteWishList} />
              }
            </h4>
              <div className={cx(s.container, s.containerPadding)}>
                { 
                  actionType != 'edit' && actionType !='delete' && <AddWishListGroupForm />
                }
                {  
                  actionType == 'edit' && <EditWishListGroupForm />
                }
                {
                  actionType == 'delete' && <DeleteWishListGroupForm initialValues = {initialValues} />
                } 
              </div>
            </div>
          </Modal.Body>
