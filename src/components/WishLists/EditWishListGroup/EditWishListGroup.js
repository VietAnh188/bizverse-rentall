...
class EditWishListComponent extends React.Component {
    ...
    render() {
        ...
        return (
            <div>
                <WishListGroupModal actionType={this.state.actionType} initialValues={initialValues} />
                <Grid fluid>
                    {
                    ...
                    }
                    {
                        !loading && getWishListGroup && <Col xs={12} sm={12} md={12} lg={12} className={cx(s.landingContent, s.noPadding, s.marginTop)}>
                            <Col xs={12} sm={12} md={12} lg={12} className={cx(s.noPadding, s.space2, s.flexBtnWishList)}>
                                <Col xs={12} sm={6} md={4} lg={4} className={s.space2}>
                                    <div className={s.innerPadding}>
                                        <Link to={"/wishlists"} className={cx(s.buttonBackAllLists,bt.btnLarge, s.resposiveBtn)}>
                                ...
                                <Col xs={12} sm={12} md={8} lg={8} className={s.space2}>
                                    <div className={cx(s.innerPadding, s.displayInlineBlock, 'shareIconRtl')}>
                                        <Button className={cx(s.button, bt.btnLarge, s.pullRight, s.noMargin, s.resposiveBtn, s.pullRghtNone)}
                                            onClick={() => {this.setState({actionType:'edit'}); openAddWishListGroupModal(initialValues, 'EditWishListGroupForm')}}>
                                            <FormattedMessage {...messages.editWishList} />
                                        </Button>
                                    </div>
                                    <div className={cx(s.displayInlineBlock, 'shareIconRtl')}>
                                        <Button className={cx(s.button, bt.btnLarge, s.pullRight, s.resposiveBtn, s.pullRghtNone, s.btnDeteleWishList)}
                                            onClick={() => { this.setState({actionType:'delete'}); openAddWishListGroupModal(initialValues, 'DeleteWishListGroupForm')}}>
                                            <FormattedMessage {...messages.deleteWishList} />
                                        </Button>
                                        {/* <DeleteWishListGroup initialValues = {initialValues} /> */}
                                    </div>
                                </Col>
                            </Col>
                                                        ...
                                                        return (
                                                            <div key={index} className={s.listingSection}>
                                                                <ListingItem
                                                                    ...
                                                                    isPayLater={item.listData.isPayLater}
                                                                />
                                                            </div>
                                                        )
                                                    }
