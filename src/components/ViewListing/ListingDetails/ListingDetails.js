...
class ListingDetails extends React.Component {
  ...
  render() {
    ...
    if (description) {
      firstArray = (description.slice(0, count)).charAt(0).toUpperCase() + (description.slice(0, count)).slice(1);
    ...
    if (data?.customRule) {
      customRules = data.customRule.split("\n").map(item => item)
    }
    ...
    return (
      <Row className={cx(s.pageContent)}>
        <div className={cx(s.horizontalLineThrough)}>
          ...
          <div>
            <p className={cx(cx(s.listingFontSize, s.justifyText))} >
        ...
        {!!customRules.length && 
          <div> 
            <div className={cx(s.horizontalLineThrough)}>
              <Row>
                <Col xs={12} sm={12} md={12} lg={12} className={cx(s.space1)}>
                  <p className={cx(s.textMutedNew)}> {formatMessage(messages.otherRules)} </p>
                </Col>
                <Col xs={12} sm={12} md={12} lg={12} className={cx(s.spaceTop1)}>
                  <Row>
                    <Col md={12} lg={12} xs={12} sm={12}>
                      {
                        customRules.map((item, key) => 
                        <div
                          key={key}
