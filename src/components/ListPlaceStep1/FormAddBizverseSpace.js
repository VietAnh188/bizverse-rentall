...
import { saveSpaceLink } from '../.../../../actions/changeSpaceLink';

class FormAddBizverseSpace extends Component {
  ...
  submitForm = async values => {
    const { listId, onAdd, currentLink, saveSpaceLink } = this.props;
    const newData = {...values, fullLink: currentLink, listId, type: 'bizverseSpace'};
    saveSpaceLink(newData);
    onAdd(newData);
  }

  render() {
    const { handleSubmit, valid, currentLink } = this.props;
    const { formatMessage } = this.props.intl;
      ...
      <div className={'inputFocusColor'}>
        <form onSubmit={handleSubmit(this.submitForm)}>
          <Field
            ...
            label={formatMessage(messages.bizveseSpaceTitle)}
            placeholder={formatMessage(messages.bizveseSpaceTitle)}
          ...
          <Field
            ...
            type="text"
            component={this.renderFormControl}
            label={formatMessage(messages.bizverseSpaceLink)}
            placeholder={formatMessage(messages.bizverseSpaceLink)}
            className='inputModal'
          />
...
FormAddBizverseSpace = reduxForm({
  form: "FormAddBizverseSpace",
  validate,
...
export default compose(injectIntl,withStyles(s, bt))(connect(mapState, mapDispatch)(FormAddBizverseSpace));