...
class HouseRules extends Component {
  ...
  renderFormControlTextArea = ({ input, label, meta: { touched, error }, children, className }) => {
    ...
    return (
      <div>
        <h3 className={s.landingContentTitle}><FormattedMessage {...messages.customRule} /></h3>
        <FormControl
          {...input}
          className={className}
          placeholder={label}
  ...
  render() {
    const { error, handleSubmit, submitting, dispatch, nextPage, previousPage, intl: { formatMessage } } = this.props;
            ...
            <div>
              ...
              <form onSubmit={handleSubmit}>
                <div className={s.landingMainContent}>
                  ...
                  <FormGroup className={s.formGroup}>
                    <Field name="customRule"
                      component={this.renderFormControlTextArea}
                      className={cx(s.textareaInput, s.bizverseBox, s.bizverseBoxSecondary)}
                    />
                  </FormGroup>
                </div>
                  ...
                  <div className={s.nextBackButton}>
                    ...
                    <FormGroup className={s.formGroup}>
                      <Col xs={12} sm={12} md={12} lg={12} className={s.noPadding}>
                        <Button className={cx(s.button, s.bizverseButtonSecondary, bt.btnLarge, s.pullLeft, 'floatRight')} onClick={() => previousPage("guest-requirements")}>
