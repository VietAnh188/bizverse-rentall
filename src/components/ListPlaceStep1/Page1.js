...
class Page1 extends Component {
  ...
  render() {
    ...
    const renderPersonCapacity = personCapacity.map(value => {
        let rows = [];
        ...
        for (let i= value.startValue; i <= value.endValue; i++) {
          rows.push(<option value={i}>{i} {i>1 ? labelMany : labelOne}</option>);
        }
        return rows;
      })
    ...
    return (
      <Grid>
        <Row>
          <Col xs={12} sm={7} md={7} lg={7} className={cx(s.landingContent, s.listSpaceContent)}>
            <Col cols={12}>
              <h2 className={s.landingTitle}><FormattedMessage {...messages.hi} />, {userData.firstName}! <FormattedMessage {...messages.letYouGetReady} />.</h2>
              <p className={s.landingStep}><span> <FormattedMessage {...messages.step1HeadingNew} /></span></p>
                ...
                <FormGroup className={s.formGroup}>
                  <Row>
                    <Col componentClass={ControlLabel} xs={12} sm={12} md={6} lg={6} >
                      <Field name="roomType" component={this.renderFormControlSelect} className={cx(s.backgroundPosition,s.formControlSelect, s.jumboSelect, s.noFontWeight, s.bizverseBox, s.bizverseBoxSecondary)}>
                        { 
                    ...
                    <Col componentClass={ControlLabel} xs={12} sm={12} md={6} lg={6} >
                      <Field name="personCapacity" component={this.renderFormControlSelect} className={cx(s.backgroundPosition, s.formControlSelect, s.jumboSelect, s.noFontWeight, s.bizverseBox, s.bizverseBoxSecondary)} >
                        { renderPersonCapacity }
                      </Field>
                    </Col>
                  </Row>
                </FormGroup>
