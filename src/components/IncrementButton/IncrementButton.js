...
class IncrementButton extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={cx(s.incrementDecrementButton, s.bizverseBox, s.bizverseBoxSecondary)}>
        <label className={s.incrementDecrementText}> {value} {label}</label>
        <Button className={cx(s.iconButton, s.bizverseBorderLeft)} onClick={() => onChange(this.decrement(value, minValue, incrementBy))}>
          <FontAwesome.FaMinus />
        </Button>
        <Button className={cx(s.iconButton, s.bizverseBorderLeft)} onClick={() => onChange(this.increment(value, maxValue, incrementBy))}>
          <FontAwesome.FaPlus />
        </Button>
      </div>
    )
