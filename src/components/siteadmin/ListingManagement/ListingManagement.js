...
class ListingManagement extends React.Component {
  ...
  handleTogglePublishListing = async (id) => {
    const { togglePublishListing } = this.props;
    const { getAllListings: { refetch } } = this.props;

    await togglePublishListing(id);
                    ...
                    <Tr key={key}>
                      ...
                      <Td data-label={formatMessage(messages.publishedLabel)} column={formatMessage(messages.publishedLabel)}>
                        <>
                          ...
                          <div>
                            <a href="javascript:void(0)" onClick={() => this.handleTogglePublishListing(value.id)} >
                              {isPublished === 'No' ?  'Publish Now' : 'Unpublish'}
                            </a>
                          </div>
                        </>
                      </Td>
                      ...
                      <Td data-label={formatMessage(messages.editLabel)} column={formatMessage(messages.editLabel)}>
                        <a href={editListing} target="_blank" rel="noreferrer" >
