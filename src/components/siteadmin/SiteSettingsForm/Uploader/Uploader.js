          ...
          >
            <div className={bt.picContainerMain}>
              <div className={bt.picContainer}>
                <div className={cx(bt.profilePic, bt.blackImg)}>
                  {
                    loading && <div><FormattedMessage {...messages.loadingLabel} /></div>
                  }
                  {
                    !loading && getLogo != null && <div 
                    style={{ backgroundImage: `url(${config.AWS_SERVICE_URL}images/logo/${getLogo.value})` }}
                    className={bt.profileImageBg}
                    />
                  }
                  {
                    !loading && getLogo === null && <div
                    style={{ backgroundImage: `url(${config.AWS_SERVICE_URL}${config.AWS_SERVICE_URL_DEFAULT_PIC})` }}
                    className={bt.profileImageBg}
                    />
                  }
                </div>
