			...
			<form onSubmit={handleSubmit(submit)}>
				...
				<FormGroup className={s.formGroup}>
					<Row>
						...
						<Col xs={12} sm={9} md={9} lg={9}>
							{
								(type != 'host' || Number(paymentMethodId) === paymentMethods.PAYPAL_PAYMENT_ID) && <span>
									{email}
								</span>
							}
							{
								type == 'host' && Number(paymentMethodId) === paymentMethods.STRIPE_PAYMENT_ID && <span>
									****{last4Digits}
								</span>
							}
						</Col>
				...
				{
					Number(paymentMethodId) === paymentMethods.PAYPAL_PAYMENT_ID && <Field
