				...
				<div className={cx({ [s.menuOpen]: this.state.isOpen == 1 }, s.sidebarWrapper, 'adminScrollBar')}>
					...
					<div className={cx("hidden-print", s.sidebarNavParent)}>
						...
						<ul className={s.sidebarNav}>
							...
							{
								validatePrivilege(3, privileges) && <li className={cx({ [s.active]: location === "/siteadmin/nfts" })}>
									<Link to={'/siteadmin/nfts'} className={cx(s.sideNavitem, s.disPlayTable)} onClick={() => this.openClose()}>
										<span className={s.disPlayTabelCell}>
											<FontAwesome.FaList className={s.navigationIcon} />
										</span>
										<span className={s.disPlayTabelCell}><FormattedMessage {...messages.manageNFT} /></span>
									</Link>
								</li>
							}
