...
class ThreadItems extends Component {
  ...
  render() {
    ...
    if (threadItems != null && threadItems.length > 0) {
      return (
        <div className={s.areaChatContent}>
          {
            data && threadItems && threadItems.length < threadItemsCount && <div className={cx(s.textCenter, s.spaceBtnSeeMore)}>
          ...
          {
            threadItems != null && threadItems.length > 0 && [...threadItems].reverse().map((item, index) => (
              <div key={index}>
