...
class Logo extends Component {
  ...
  render() {
    ...
    let defaultLogoHeight =
      logoHeight && logoHeight != null ? logoHeight : "63";
    let defaultLogoWidth = logoWidth && logoWidth != null ? logoWidth : "250";
    let defaultHomeLogoHeight =
      homeLogoHeight && homeLogoHeight != null ? homeLogoHeight : "63";
    let defaultHomeLogoWidth =
      homeLogoWidth && homeLogoWidth != null ? homeLogoWidth : "63";
    ...
    return (
      <Link to={link} className={className}>
        {homeLogo   && location === "/" && !logoImage  && layoutType != 2 && (
          <img
            src={config.AWS_SERVICE_URL+"images/logo/logoBiz.png"}
            className={cx(!showMenu ? "displayBlock" : "displayNone")}
        ...
        {homeLogo   && location === "/" && logoImage  && layoutType != 2 && (
          <img
            src={config.AWS_SERVICE_URL+"images/logo/" + homeLogo}
            className={cx(!showMenu ? "displayBlock" : "displayNone")}
            height={defaultHomeLogoHeight}
            width={defaultHomeLogoWidth}
          />
        ...
        {homeLogo   && location === "/" && logoImage  && layoutType == 2 && (
          <img
            src={config.AWS_SERVICE_URL+"images/logo/" + logoImage}
            className={cx(!showMenu ? "displayBlock" : "displayNone")}
        ...
        {!homeLogo   && !logoImage  && siteName != null && location !== "/" && (
          <span className={cx(s.logoColor)}>{siteName}</span>
        )}
        {!homeLogo   && !logoImage  && siteName != null && location === "/" && (
          <span className={cx(!showMenu ? s.whiteColor : s.logoColor)}>
            {siteName}
          </span>
        )}
        {!homeLogo   && !logoImage  && siteName === null && (
          <span className={s.logoColor}>Site Name</span>
        )}
        {logoImage  && location?.length > 1 && (
          <img
            src={config.AWS_SERVICE_URL+"images/logo/" + logoImage}
            height={defaultLogoHeight}
            width={defaultLogoWidth}
        ...
        {!logoImage  && homeLogo   && location !== "/" && siteName != null && (
          <span className={cx(s.logoColor)}>{siteName}</span>
        )}
      </Link>
    );
