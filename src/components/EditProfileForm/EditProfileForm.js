...
// Actions
import { syncBizverseSocial } from '../../actions/user/syncBizverseSocial';
...
class EditProfileForm extends Component {
  ...
  handleSyncBizverseSocial = () => {
    const { syncBizverseSocial } = this.props;
  ...
  render() {
    ...
    const { formatMessage } = this.props.intl;
    const { siteSettingStatus, syncBizverseSocial } = this.props;
    ...
    return (
      <div className={cx('inputFocusColor', 'commonListingBg', 'inputFocusColorEditRTL', 'edit-profile-form-container')}>
        {error && <span className={s.errorMessage}>{formatMessage(error)}</span>}
        <Panel className={cx(s.panelHeader)}>
          <div>
            <h3 className={s.editFormHeadline}>{title}</h3>
          </div>
          <Form onSubmit={handleSubmit(submit)}>
            <Row className={s.formGroup}>
              ...
              <Col componentClass={ControlLabel} xs={12} sm={9} md={9} lg={9}>
                <Field name="firstName"
                  ...
                  className={cx(bt.commonControlInput, s.bizverseBox, s.bizverseBoxSecondary)}
                />
              </Col>
            </Row>
            ...
            <Row className={s.formGroup}>
              <Col componentClass={ControlLabel} xs={12} sm={3} md={3} lg={3} className={cx(s.textAlign, 'textAlignLeftRtlEdit')}>
                <label className={s.labelText} >{formatMessage(messages.wallet)}</label>
              </Col>
              <Col componentClass={ControlLabel} xs={12} sm={9} md={9} lg={9}>
              <Field name="wallet"
                  type="text"
                  isDisabled={true}
                  component={this.renderFormControl}
                  label={formatMessage(messages.wallet)}
                  className={cx(bt.commonControlInput, s.bizverseBox, s.bizverseBoxSecondary)}
                  placeholder="e.g. Paris, France /Brooklyn, NY, IL"
                />
              </Col>
            ...
            <Row>
              ...
              <Button 
                bsSize="small" 
                className={cx(bt.btnPrimary, bt.btnLarge, s.btnSync)} 
                type="button"
                onClick={this.handleSyncBizverseSocial}
              >
                {formatMessage(messages.syncBizverseSocialData)}
              </Button>
