...
class ListingPhotos extends React.Component {
  ...
  render() {
    const { id, listPhotos, coverPhoto, size, formatURL, title, isMapShow, arrow, personalizedURL, rtl, isPayLater } = this.props;
    const { load } = this.state;
    const imagepath = `${config.AWS_SERVICE_URL}images/upload/${size}`;
    const placeholderPath = `${config.AWS_SERVICE_URL}images/upload/placeholder_`;
        ...
        }
        {
          load && <Swiper {...params} rtl={rtl} ref={node => this.swiper = node !== null ? node.swiper : null}>
          {
            listPhotos && listPhotos.length && listPhotos.map((item, itemIndex) => {
              ...
              let currentImagePath = imagepath + item.name;
                        ...
                        <div className={s.content}>
                          {item.isPanorama ? (
                            <> 
                              ...
                              <ImagePanorama 
                                  height="100%" 
                                  className={cx(s.imageContent, 'imgParonama')} 
                                  width="100%"
                              ...
                              <div className={'imgPanoramaResponsive'}>
                                <div data-background={currentImagePath} className={cx(s.imageContent, "swiper-lazy")}>
