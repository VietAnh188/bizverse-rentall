...
class SaveCalendar extends Component {
  ...
  renderDateRange = ({ input, meta: { touched, error }, formName, numberOfMonths, index, defaultStartDate, defaultEndDate, isCurrentStatus, resetCalendar }) => {
    const { formatMessage } = this.props.intl;
  ...
  async handleSave() {
    const { change, selectedDays } = this.props;
  ...
  render() {
    ...
    return (
      <div>
        {
          (start || isStartDate) && <Col lg={12} md={12} sm={12} xs={12} className={cx(c.saveCalendarContainer, c.CalendarPopup, c.bizverseBox, c.bizverseBoxSecondary)}>
            <div className={c.innerCPopup}>
              <FormGroup className={cx(c.formGroup, c.textLeft)}>
                <button bsStyle="link"
                  className={cx(c.noPadding, c.space3, c.noBorderBtn)}
                  onClick={() => { resetCalendar() }}>
                  <svg viewBox="0 0 24 24" role="img"
                    style={{ height: '16px', width: '16px', display: 'block', fill: '#000', background: '#fff' }}>
...

              {
                isCurrentStatus == 1 && <FormGroup className={cx(c.formGroup, c.buttonLeft, 'saveRightRtl')}>
                  <Loader
                    ...
                    buttonType={"button"}
                    show={isLoading}
                    className={cx(c.btnPrimaryCalendar, c.btnlarge)}
                    disabled={isLoading}
              ...
              <Button
                className={cx(c.btnCancelCalendar, c.btnlarge, c.buttonRight)}
                onClick={() => { resetCalendar() }}
              >
