...
class Inbox extends React.Component {
  ...
  render() {
    ...
    if (type === 'host') {
      hostActive = cx(s.active, s.bizverseTabActive);
    } else {
      guestActive = cx(s.active, s.bizverseTabActive);
    }
    ...
    return (
      <div className={cx(s.progressContainerResponsive, s.space8, 'commonListingBg', 'tripContainer')}>
        <Grid fluid>
          <Row className={cx(s.containerResponsive)}>
            <Col xs={12} sm={12} md={4} lg={4}>
              <div className={cx(s.slideShow, s.space3, s.hideSm)}>
                <Avatar
                  ...
                  height={190}
                  width={190}
                  className={cx(s.imgResponsive, s.bizverseBox)}
                />
              </div>
            ...
            <Col xs={12} sm={12} md={8} lg={8} className={cx(s.h4,s.backgroundInboxMess)}>
              <ul className={cx("list-inline", 'inbox-tabs', s.tabs, s.bizverseTabs)}>
                <li className={cx({[s.bizverseTabActiveBorderContainer]: !isHost})}>
                  <a
                    className={cx(s.bizverseTab, {
                      [s.bizverseTabActive]: !isHost
                    })}
              ...
              <Row className={GetAllThreads && GetAllThreads.threadsData && GetAllThreads.threadsData.length > 0 ? cx(s.containerResponsive, s.scrollInbox) : s.containerResponsive}>
                <Col xs={12} sm={12} md={12} lg={12}>
                  <Panel className={s.panelHeader}>
                    ...
                    {
                      !!GetAllThreads?.threadsData && GetAllThreads.threadsData.length > 0
                      && <div>
                        <CustomPagination
