...
import { fileuploadDir, deepLinkBundleId, profilePhotoUploadDirS3 } from '../config';
...
const mobileRoutes = app => {
    ...
    try {
      ...
      if (requestHeader && ((requestHeader.isAuth === true && isLoggedInUser) || !requestHeader.isAuth)) {
        const userLogin = await User.findOne({
          ...
          where: { id: isLoggedInUser.id, userDeletedAt: null },
          raw: true
        });
        if (userLogin) {
          where = {
  ...
  app.post('/uploadPhoto', upload.single('file'), async (req, res, next) => {
    let file = req.file;
    let requestHeader = req.headers;
    let isLoggedInUser;
    let status, errorMessage, path, filename;
    ...
    try {
      ...
      if (userLogin) {
        ...
        status = 200;
        filename = photoName;
        path = profilePhotoUploadDirS3 + photoName;
      } else {
        status = 400;
        errorMessage = 'Sorry user not exist in database!';
      }
