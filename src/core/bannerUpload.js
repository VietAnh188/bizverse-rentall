...
import { banneruploadDir, homebanneruploadDir, bannerUploadDirS3 } from '../config';
...
const bannerUpload = app => {
  ...
  app.post('/uploadBanner', function (req, res, next) {
    ...
    .toBuffer()
    .then(async data => {
      const awsFileName = bannerUploadDirS3 + file.filename
    ...
    sharp(file.path)
      ...
      .toBuffer()
      .then(async data => {
        const awsFileName = bannerUploadDirS3 + 'x_' + file.filename
        const results = await s3UploadSingleFile(awsFileName, data); 
        return results
      })
