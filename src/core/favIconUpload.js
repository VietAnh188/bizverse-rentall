...
import { faviconUploadDir, faviconUploadDirS3 } from '../config';
...
const favIconUpload = app => {
    ...
    app.post('/uploadFavIcon', function (req, res, next) {
        ...
        try {
            ...
            .toBuffer()
            .then(async data => {
                const awsFileName = faviconUploadDirS3 + 'favicon.png';
            ...
            .toBuffer()
            .then(async data => {
                const awsFileName = faviconUploadDirS3 + file.filename;
                const results = await s3UploadSingleFile(awsFileName, data); 
                return results
            })
            ...
            await Promise.all([
                data.map(async (o) => {
                    await sharp(file.path)
                        ...
                        .toBuffer()
                        .then(async data => {
                            const awsFileName = faviconUploadDirS3 + o.filename;
                            const results = await s3UploadSingleFile(awsFileName, data); 
                            return results
                        })
                })
