...
import { locationuploadDir, locationUploadDirS3 } from '../config';
...
const locationUpload = app => {
  ...
  app.post('/uploadLocation', function (req, res, next) {
    ...
    const defaultImage = await sharp(file.path).toBuffer().then(async data=>{
        const awsFileName = locationUploadDirS3 + file.filename
        const results = await s3UploadSingleFile(awsFileName, data); 
        return results
    })
    // small - 101 * 67
    const smallImage = await sharp(file.path)
        .rotate()
        .resize(250, null)
        .toBuffer()
        .then(async data => {
          const awsFileName = locationUploadDirS3 + 'small_' + file.filename
          const results = await s3UploadSingleFile(awsFileName, data); 
          return results
        })
    ...
    if (file.mimetype === 'image/jpeg') {
      // x_medium - 450 * 300  
      const mediumImage = await sharp(file.path)
          .rotate()
          .resize(450, null)
          .jpeg({ quality: 50 })
          .toBuffer()
          .then(async data => {
            const awsFileName = locationUploadDirS3 + 'medium_' + file.filename
            const results = await s3UploadSingleFile(awsFileName, data); 
            return results 
          })
