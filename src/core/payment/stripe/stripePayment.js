...
const stripePayment = app => {
    app.post('/stripe-reservation', async function (req, res) {
        ...
        if (status === 200) {
            try {
                ...
                const convertedAmount = STRIPE_ZERO_DECIMAL_CURRENCIES.includes(reservationDetails.currency.toUpperCase()) ? reservationDetails.amount : reservationDetails.amount * 100
        ...
        if (status === 200 && intent && 'id' in intent) {
            ...
            console.log("-------------- intern ------------------------", intent)
            
            handleOnPaymentSuccess({
                reservationId,
                payerEmail: guestEmail,
                payerId: customerId,
                // receiverEmail: intent.id,
                // receiverId: intent.id,
        ...
        let redirect = '/users/trips/itinerary/' + reservationId;
    ...
    app.get('/stripe-get-customer', async function (req, res) {
        await stripe.customers.retrieve('cus_Dn7eqHDkO4Sy3b');
