      ...
      if (req.user && req.user.admin == true) {
        ...
        if (status == 200) {
          ...
          paypal.payout.create(create_payout_json, sync_mode, async function (error, payout) {
            if (error) {
              res.send({ status: error.response, errorMessage: error.response && error.response.message + " "+ (error.response && error.response.details ? error.response.details[0]?.issue: "") });
              throw error;
            } else {
