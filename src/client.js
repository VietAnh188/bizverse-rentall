...
import { setMaintenance } from './actions/setMaintenance';
...
let arrResponse = [];
context.client.networkInterface.useAfter ([{
  applyAfterware ({response}, next){
    arrResponse.push(response);
    const status = (e) => e.status === 503;
    if(arrResponse.some(status)){
      store.dispatch(setMaintenance({name: 'isMaintenance', value: true}))
    } else {
      store.dispatch(setMaintenance({name: 'isMaintenance', value: false}))
    }
    next()
  }}])
