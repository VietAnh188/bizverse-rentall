import searchListingType, { viewTypeEnumType } from '../types/Listing/SearchListingType';
...
const SearchListing = {
  ...
  args: {
    ...
    viewType: { type: viewTypeEnumType }
  },
  ...
  async resolve ({ request }, {
    ...
    try {
      ...
      let publishedFilter = { isPublished: true, isReady: true };
      ...
      async function getListingIdRelatedRecommended() {
        const instances = await Listing.findAll({
          where: {
            isPublished: true
          },
          include: [
        ...
        return instances.map(instance => instance.dataValues.id);
      }

      async function getListingIdRelatedListView() {
        const instances = await Listing.findAll({
          where: {
            isPublished: true
          },
      ...
      // View Type Filter
      switch (viewType) {
        case viewTypeEnumType.getValue(VIEW_TYPES.recommended).value:
          const listingIdRelatedRecommended = await getListingIdRelatedRecommended();
          viewTypeFilter = {
            id: {
              $in: [
                ...listingIdRelatedRecommended
              ]
            }
          };
          break;
        case viewTypeEnumType.getValue(VIEW_TYPES.mostViewed).value:
          const listingIdRelatedListView = await getListingIdRelatedListView();
          viewTypeFilter = {
            id: {
              $in: [
                ...listingIdRelatedListView
              ]
            }
          };
          break;
