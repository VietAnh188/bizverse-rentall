...
export function searchListing({ viewType, personCapacity, dates, geography, currentPage, geoType, lat, lng, sw_lat, sw_lng, ne_lat, ne_lng, location }) {

  return async (dispatch, getState, { client }) => {

    dispatch({ type: SEARCH_LISTING_START });
    dispatch(reset('SearchForm'));
    dispatch({type: CHANGE_VIEWTYPE, payload: {viewType}})
    ...
    try {
      ...
      if (data.SearchListing) {
        ...
        await dispatch(change('SearchForm', 'viewType', viewType));
        // Default Map Show
