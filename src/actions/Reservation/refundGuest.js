    ...
    try {
      ...
      if (paymentMethodId == 1) {
        ...
        const { status } = await refundToGuest(
          reservationId, receiverEmail, receiverId, payerEmail, payerId, convertedAmount.toFixed(2), paymentCurrency, payEmail
        );

        if (status === 'PENDING' || status === 'PROCESSING' || status === 'SUCCESS') {
          dispatch({
