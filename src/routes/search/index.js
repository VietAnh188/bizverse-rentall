...
export default async function action({ params, store, query }) {
  ...
  const viewTypeParam = getParam('viewtype', 'all');

  store.dispatch(change('SearchForm', 'viewType', viewTypeParam))

  await store.dispatch(searchListing({ viewType: viewTypeParam, personCapacity, dates, geography, currentPage, geoType, lat, lng, sw_lat, sw_lng, ne_lat, ne_lng, location }))
