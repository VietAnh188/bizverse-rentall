...
import BookedDatesQuery from './BookedDates.graphql';
...
// Actions
import { setCurrentViewListing } from '../../actions/Listing/currentListing'
...
class ViewListing extends React.Component {
  ...
  componentDidUpdate() {
    const { setCurrentViewListing, getListingData: { UserListing } } = this.props;

    if (UserListing) {
      setCurrentViewListing(UserListing)
    }
  }

  render() {
    const { listId, title, getListingData: { loading, UserListing }, preview, averageBasePrice } = this.props;
    const { ListingBlockedDates, ListingBookedDates } = this.props;
    ...
    // Define blocked dates = host blocked dates + listing has booked dates
    let blockedDates = [];

    if (ListingBlockedDates.UserListing) {
      blockedDates = ListingBlockedDates.UserListing.blockedDates
    }

    if (ListingBookedDates?.getListBookedDates?.bookedDates?.length) {
      blockedDates = [...blockedDates, ...ListingBookedDates.getListBookedDates.bookedDates]
    }

    const isLoadingCalendar = ListingBlockedDates.loading || ListingBookedDates.loading;

    return (
      <div className={cx(s.root, 'bizverse-container', 'view-listing-container')}>
              ...
              <Element name="test1" className="element">
                <Grid fluid className={cx(s.horizontalLineThrough)}>
                  <Row className={cx(s.pageContent)}>
                    <Col xs={12} sm={12} md={8} lg={8} className={cx(s.bizverseBox)}>
                      <Row>
                        <div className={s.stickyContainer}>
                          <AutoAffix viewportOffsetTop={0} container={this} affixClassName={s.showAffix}>
                            <div className={cx(s.zPanel, s.panelDefault, s.bizverseBorder, s.bizverseBorderBottom)}>
                        ...
                        <Grid fluid className={cx(s.noBorder, s.paddingOverview)}>
                          <ListingDetails
                            data={UserListing}
                            isHost={isHost}
                    ...
                    <Col xs={12} sm={12} md={4} lg={4} className={cx(s.positionSticky)}>
                      {
...
export default compose(
  ...
  graphql(BookedDatesQuery,
    {
      name: 'ListingBookedDates',
      options: (props) => ({
        variables: {
          listId: props.listId
        },
